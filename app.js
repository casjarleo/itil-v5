...
var R_ALL=[].concat(questionsD1,questionsD2,questionsD3,questionsD4,questionsD5,questionsD6,questionsD7);
var ALL=[];
for(var i=0;i<R_ALL.length;i++){var r=R_ALL[i];ALL.push({d:r[0],id:'d'+r[0]+'_'+r[1],q:[r[2],r[3]],o:[r[4],r[5]],c:r[6],e:[r[7],r[8]]});}
...
function restoreExam(state){
 ...
 examQs=[];
 var idMap={};
 for(var i=0;i<ALL.length;i++) idMap[ALL[i].id]=ALL[i];
 for(var i=0;i<state.examQIds.length;i++){
 ...
 var d=lastDomain,pct=Math.round((DCOUNTS[d]/ALL.length)*100);
 ...
 document.getElementById("examInfo").innerHTML=info;
...
function toggleTheme(){
 ...
 document.getElementById("navTheme").textContent=T===0?"☀️":"🌙";
 safeLSSet("itilv5_theme",""+T);
}
function toggleLang(){
 L=L===0?1:0;
 document.getElementById("navLang").textContent=L===0?"🇪🇸":"🇬🇧";
 safeLSSet("itilv5_lang",""+L);
 document.documentElement.lang=L===0?"es":"en";
 refreshUI();
 if(document.getElementById("exam").className.indexOf("active")!==-1&&examQs.length>0){renderQ();renderShortcuts();updateBackBtn();renderExamCounter();}
...
function refreshUI(){
 document.getElementById("mainTitle").innerHTML=UI.title[L];
 document.getElementById("navInfo").textContent=UI.info[L];
 document.getElementById("navHome").textContent=UI.home[L];
 document.getElementById("navHist").textContent=UI.hist[L];
 document.getElementById("navTheme").textContent=T===0?"☀️":"🌙";
 document.getElementById("btnSim").textContent=UI.simBtn[L];
 document.getElementById("btnFull").textContent=UI.fullBtn[L]+" ("+ALL.length+" Qs)";
 document.getElementById("btnDom").textContent=UI.domBtn[L];
 document.getElementById("btnSmart").textContent=UI.smartBtn[L];
 document.getElementById("btnFlash").textContent=UI.flashBtn[L];
 document.getElementById("btnAdapt").textContent=UI.adaptBtn[L];
 document.getElementById("btnPrev").textContent=UI.prev[L];
 document.getElementById("btnNext").textContent=UI.next[L];
 document.getElementById("btnFlag").textContent=UI.flag[L];
 document.getElementById("btnFinish").textContent=UI.finish[L];
 document.getElementById("btnBackHome").textContent=UI.backHome[L];
 document.getElementById("btnReview").textContent=UI.review[L];
 document.getElementById("histTitle").textContent=UI.histTitle[L];
 document.getElementById("btnClearHist").textContent=UI.clearHist[L];
 document.getElementById("btnResetStats").textContent=UI.resetStats[L];
 document.getElementById("domTitle").textContent=UI.domTitle[L];
 document.getElementById("btnBackDom").textContent=UI.backDom[L];
 document.getElementById("summaryTitle").textContent=UI.summaryTitle[L];
 document.getElementById("progressTitle").textContent=UI.progressTitle[L];
 document.getElementById("btnFlashPrev").textContent=UI.prev[L];
 document.getElementById("btnFlashNext").textContent=UI.next[L];
 document.getElementById("btnBackFlash").textContent=UI.flashBack[L];
 document.getElementById("globalFooter").innerHTML=UI.infoFooter[L].replace(/\n/g,"<br>");
 ...
 document.getElementById("navTheme").title=T===0?(L===0?"Cambiar a modo claro":"Switch to light mode"):(L===0?"Cambiar a modo oscuro":"Switch to dark mode");
 document.getElementById("navLang").title=L===0?"Switch to English":"Cambiar a Español";
}
function renderSummary(){
 var hist=JSON.parse(safeLSGet("itilv5_history","[]"));
 var exams=hist.length,best=0;
 for(var i=0;i<hist.length;i++){if(hist[i].pct>best) best=hist[i].pct;}
 var items=[
 {icon:"🏆",num:exams,color:"var(--purple)",label:UI.examsDone[L]},
 {icon:"📊",num:best+"%",color:best>=65?"var(--green)":"var(--red)",label:UI.bestScore[L]}
 ];
 var h='<div class="sum-row">';
...
function renderDomainCards(){
 ...
 h+='<div class="dom-card" onclick="startDomain('+d+')" style="border-left:4px solid '+DCOLORS[d]+'">';
...
function renderSimLanding(){
 ...
 document.getElementById("slStart").textContent=UI.slStart[L];
 document.getElementById("slBack").textContent=UI.slBack[L];
 document.getElementById("slRealLabel").textContent=UI.slRealLabel[L];
 var h="";
 for(var d=0;d<7;d++){
 var w=Math.round((DSIM[d]/40)*100),pct=Math.round((DCOUNTS[d]/ALL.length)*100);
 ...
 document.getElementById("slDomains").innerHTML=h;
}
...
function renderInfo(){
 document.getElementById("infoSub").textContent=UI.infoSub[L];
  var discEl=document.getElementById("infoDisclaimer");
 ...
 var fh="";
 for(var i=0;i<FEATS.length;i++) fh+='<div class="feat-card"><strong>'+FEATS[i][L][0]+'</strong><br><span>'+FEATS[i][L][1]+'</span></div>';
 document.getElementById("infoFeatList").innerHTML=fh;
 document.getElementById("infoKeyTitle").textContent=UI.infoKeyTitle[L];
 document.getElementById("infoKC1").textContent=UI.infoKC1[L];
 document.getElementById("infoKC2").textContent=UI.infoKC2[L];
 var kh="";
...
function renderExamCounter(){
 ...
 var t=ans+"/"+examQs.length+" "+UI.answered[L];
 if(fl>0) t+=" · "+fl+" 🚩";
...
function init(){
 ...
 var simTotal=40,assigned=0;
 DSIM=[0,0,0,0,0,0,0];
 for(var d=0;d<7;d++){DSIM[d]=Math.max(1,Math.round((DWEIGHT[d]/100)*simTotal));assigned+=DSIM[d];}
 while(assigned>simTotal){for(var d=6;d>=0&&assigned>simTotal;d--){if(DSIM[d]>1){DSIM[d]--;assigned--;}}}
 while(assigned<simTotal){for(var d=0;d<7&&assigned<simTotal;d++){DSIM[d]++;assigned++;}}
 var savedT=safeLSGet("itilv5_theme",null);
 if(savedT==="1"){T=1;document.body.className="light";}
 var savedL=safeLSGet("itilv5_lang",null);
...
function showView(v){
 ...
 document.getElementById(v).className="view active";
 var ids=["navHome","navHist","navInfo"];
 var active=-1;
 if(v==="home"||v==="domains"||v==="simLanding") active=0;
 else if(v==="history") active=1;
 else if(v==="info") active=2;
 for(var i=0;i<ids.length;i++) document.getElementById(ids[i]).className=i===active?"active":"";
...
function startDomain(d){
 ...
 document.getElementById("timerDiv").style.display="none";
 var pct=Math.round((DCOUNTS[d]/ALL.length)*100);
 document.getElementById("examInfo").innerHTML=DICONS[d]+' <strong>'+DFULL[d][L]+'</strong> · ITIL v5 · '+UI.domLabel[L]+' '+(d+1)+' ('+pct+'%) · '+examQs.length+' '+UI.qsLabel[L]+'<div class="exam-topics">'+DTOPICS[d][L]+'</div>';
 updateBackBtn();renderShortcuts();
 showView("exam");renderQ();renderGrid();renderExamCounter();
}
...
function renderFlashCard(){
 var q=flashQs[flashIdx],letters=["A","B","C","D"];
 var h='<div style="font-size:1.15em;font-weight:600;margin-bottom:15px">'+q.q[L]+'</div>';
 if(flashFlipped){
 ...
 document.getElementById("flashCard").innerHTML=h;
 document.getElementById("flashCounter").textContent=(flashIdx+1)+" / "+flashQs.length;
}
...
function startTimer(){
 ...
 document.getElementById("timerDiv").textContent="⏱ "+pad(m)+":"+pad(s);
 document.getElementById("timerDiv").style.color=timeLeft<300?"var(--red)":"var(--orange)";
 },1000);
}
...
function renderQ(){
 ...
 var qhtml=txt.replace(/\[\?\]/g,'<span class="missing-word">[?]</span>');
 ...
 document.getElementById("qCounter").textContent=(curQ+1)+" / "+examQs.length;
 document.getElementById("progBar").style.width=Math.round(((curQ+1)/examQs.length)*100)+"%";
 updateGrid();
}
...
function submitExam(){
 ...
 if(timerID){clearInterval(timerID);timerID=null;}
 examActive=false;clearAutoSave();
 var correct=0,total=examQs.length,dc={},dt={};
 for(var d=0;d<7;d++){dc[d]=0;dt[d]=0;}
 for(var i=0;i<examQs.length;i++){var qObj=examQs[i];dt[qObj.d]++;if(answers[i]===qObj.c){correct++;dc[qObj.d]++;}}
 var pct=Math.round((correct/total)*100),pass=pct>=65;
 var hist=JSON.parse(safeLSGet("itilv5_history","[]"));
 var now=new Date();
 var modeStr="";
 if(examMode==="sim"&&realMode) modeStr=UI.realLabel[L];
 else if(examMode==="sim") modeStr=UI.simMode[L];
 else if(examMode==="full") modeStr=UI.fullMode[L];
 else if(examMode==="smart") modeStr=UI.smartMode[L];
 else if(examMode==="adaptive") modeStr=UI.adaptMode[L];
 else modeStr=UI.domMode[L];
 var totalTime=0,ac=0;
 for(var i=0;i<qTimes.length;i++){if(qTimes[i]>0){totalTime+=qTimes[i];ac++;}}
 var avgT=ac>0?Math.round(totalTime/ac):0;
 ...
 for(var d=0;d<7;d++){
 if(dt[d]===0) continue;
 var dp=Math.round((dc[d]/dt[d])*100),col="var(--red)";
 ...
 h+='<div class="pct" style="color:'+col+'">'+dp+'% ('+dc[d]+'/'+dt[d]+')</div></div>';
 ...
 if(str.length>0){h+='<h3 style="color:var(--green)">'+UI.strengths[L]+'</h3><ul>';for(var i=0;i<str.length;i++) h+='<li>'+str[i]+'</li>';h+='</ul>';}
 if(wk.length>0){h+='<h3 style="color:var(--red)">'+UI.improve[L]+'</h3><ul>';for(var i=0;i<wk.length;i++) h+='<li>'+wk[i]+'</li>';h+='</ul>';}
 h+='</div>';
 }
 h+=buildDomainPerf();
...
function buildDomainPerf(){
 ...
 var tc={},tt={};
 for(var d=0;d<7;d++){tc[d]=0;tt[d]=0;}
 for(var i=0;i<hist.length;i++){var e=hist[i];if(e.dc&&e.dt){for(var d=0;d<7;d++){tc[d]+=(e.dc[d]||0);tt[d]+=(e.dt[d]||0);}}}
 var hasData=false;
 for(var d=0;d<7;d++){if(tt[d]>0) hasData=true;}
 if(!hasData) return "";
 var h='<div class="results-box"><h2>'+UI.domPerf[L]+'</h2>';
 for(var d=0;d<7;d++){
 if(tt[d]===0) continue;
 var dp=Math.round((tc[d]/tt[d])*100),col="var(--red)";
 if(dp>=80) col="var(--green)";else if(dp>=65) col="var(--orange)";
 h+='<div class="domain-bar"><div class="name">'+DICONS[d]+' '+DN[d][L].split(" ").slice(0,2).join(" ")+'</div>';
 h+='<div class="bar-bg"><div class="bar-fill" style="width:'+dp+'%;background:'+col+'"></div></div>';
...
function renderProgress(){
 ...
 var q=ALL[i],s=qs[q.id],cls="prog-dot ",tip="";
 if(!s||s.attempts===0){cls+="unseen";unseen++;tip="#"+q.id;}
 else{
 var r=s.correct/s.attempts;
...
function renderHistory(){
 ...
 var html='<table><thead><tr><th>#</th><th>'+UI.date[L]+'</th><th>'+UI.mode[L]+'</th><th>'+UI.questions[L]+'</th><th>'+UI.correct[L]+'</th><th>'+UI.score[L]+'</th><th>'+UI.status[L]+'</th></tr></thead><tbody>';
 for(var i=h.length-1;i>=0;i--){
 var r=h[i],sc=r.pct>=65?"var(--green)":"var(--red)";
 ...
 var bH=Math.max(h[i].pct,5),col=h[i].pct>=65?"var(--green)":"var(--red)";
...
document.addEventListener("keydown",function(e){
 ...
 if(document.getElementById("flashcards").className.indexOf("active")!==-1){
