...
function init(){
  ...
  DSIM=[0,0,0,0,0,0,0];
  for(var d=0;d<7;d++){DSIM[d]=Math.max(1,Math.round((DCOUNTS[d]/ALL.length)*simTotal));assigned+=DSIM[d];}
...
function showView(v){
  ...
  var ids=["navInfo","navHome","navHist"];
  var active=-1;
  if(v==="info") active=0;
  else if(v==="home"||v==="domains"||v==="simLanding") active=1;
  else if(v==="history") active=2;
